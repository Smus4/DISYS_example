...
type TemplateClient interface {
	...
	// many messages are sent and one is recieved
	GetTime(ctx context.Context, opts ...grpc.CallOption) (Template_GetTimeClient, error)
}
...
func (c *templateClient) GetTime(ctx context.Context, opts ...grpc.CallOption) (Template_GetTimeClient, error) {
	stream, err := c.cc.NewStream(ctx, &Template_ServiceDesc.Streams[1], "/proto.Template/GetTime", opts...)
	if err != nil {
		return nil, err
	}
	x := &templateGetTimeClient{stream}
	return x, nil
}

type Template_GetTimeClient interface {
	Send(*TimeRequest) error
	CloseAndRecv() (*Time, error)
	grpc.ClientStream
}

type templateGetTimeClient struct {
	grpc.ClientStream
}

func (x *templateGetTimeClient) Send(m *TimeRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *templateGetTimeClient) CloseAndRecv() (*Time, error) {
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
	m := new(Time)
...
type TemplateServer interface {
	...
	// many messages are sent and one is recieved
	GetTime(Template_GetTimeServer) error
...
func (UnimplementedTemplateServer) GetTime(Template_GetTimeServer) error {
...
func _Template_GetTime_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(TemplateServer).GetTime(&templateGetTimeServer{stream})
}

type Template_GetTimeServer interface {
	SendAndClose(*Time) error
	Recv() (*TimeRequest, error)
	grpc.ServerStream
}

type templateGetTimeServer struct {
	grpc.ServerStream
}

func (x *templateGetTimeServer) SendAndClose(m *Time) error {
	return x.ServerStream.SendMsg(m)
}

func (x *templateGetTimeServer) Recv() (*TimeRequest, error) {
	m := new(TimeRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
