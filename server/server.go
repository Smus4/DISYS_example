...
import (
	...
	gRPC "github.com/Smus4/DISYS_example/proto"
...
func launchServer() {
	...
	//list, err := net.Listen("tcp", fmt.Sprintf("localhost:%s", *port)) //setsup local host
	list, err := net.Listen("tcp", fmt.Sprintf(":%s", *port)) //sets up remote server
	if err != nil {
...
func (s *Server) GetTime(msgStream gRPC.Template_GetTimeServer) error {
	for {
		...
		// log the message
		log.Printf("Received request %s", msg.Message)
	}

	// be a nice server and say goodbye to the client :)
	Time := &gRPC.Time{Message: time.Now().GoString()}
	// time.Sleep(1000*time.Millisecond); // venter 1000 sekunder
	msgStream.SendAndClose(Time)
...
// import (
// 	template "github.com/Smus4/DISYS_example"
// )
...
// func (s *Server) Course(context.Context, *MsgName1) (*Ack, error) {
...
// func (s *Server) Student(Template_StudentServer) error {
...
// func (s *Server) Teacher(Template_TeacherServer) error {
